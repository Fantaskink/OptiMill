...
#define PRICE_PER_KM 200000        // Price for cable digging being 200kr pr. meter
#define TRANSPORT_COST 100000      // Approximate transport cost for any given wind turbine.
...
typedef struct Windmill
{
    ...
    int price;     // Price of the rotor blades and tower, in Danish Crowns
...
typedef struct Area
{
    ...
    double inv_return;        // Monetary return of specific wind turbine when placed in specific area
} Area;
...
void print_windmill_investment_return(Area area, Windmill windmill)
{
    ...
    printf("Dette svarer til at vindmøllen tjener: %.2lf mio. kr. om året\n", yearly_income);
